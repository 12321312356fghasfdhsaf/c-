// Быки_коровы2.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
// Распихать цифры из числа value в массив ar
...
int main()
{
	...
	do {
		...
		for (int i = 0; i < size; i++)
		{
			if (auto_number[i] == user_number[i])
			{
				bools++;
			}
			for (int j = 0; j < size; j++)
			{
				if (auto_number[i] == user_number[j])
				{
					cows++;
				}
			}
